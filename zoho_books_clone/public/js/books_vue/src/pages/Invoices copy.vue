...
async function fetchItems(q = "") {
  try {
    ...
    const r = await apiList("Item", {
      fields: ["name","item_name","standard_rate","stock_uom","description","hsn_code"],
      filters: f, limit: 50, order: "item_name asc",
    }) || [];
    items.value = r.map(x => ({
      ...x,
      value: x.name,
...
async function onItemChange(line) {
  ...
  if (it) {
    ...
    line.rate       = flt(it.standard_rate || it.rate);
    line.uom        = it.stock_uom || it.uom || "Nos";
  ...
  if (line.item_code) {
    try {
      ...
      else if (doc?.hsn_code) line.hsn_code   = doc.hsn_code;
