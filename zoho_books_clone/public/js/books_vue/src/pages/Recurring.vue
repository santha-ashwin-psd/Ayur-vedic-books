  ...
  <div class="rec-page">
    <div class="rec-actions">
      <div class="rec-search-wrap">
        ...
        <input v-model="search" placeholder="Search subscriptions…" class="rec-search-input" />
      </div>
      <div class="rec-pills">
      ...
      <div style="display:flex;gap:8px;margin-left:auto">
        ...
        <button class="rec-btn-primary" @click="openNew"><span v-html="icon('plus',13)"></span> New Subscription</button>
      </div>
    </div>
      ...
      <table class="rec-table">
        ...
        <tbody>
          ...
          <template v-else>
            ...
            <tr v-if="!sorted.length"><td colspan="8" class="rec-empty">No subscriptions found</td></tr>
          </template>
        </tbody>
      </table>
    </div>
    ...
    <div class="rec-drawer" :class="{open:drawerOpen}">
      <div class="rec-dheader">
        <div class="rec-dheader-title">New Recurring Subscription</div>
      ...
      <div class="rec-dbody">
        <div class="rec-fields-grid">
          <div class="rec-field" style="grid-column:1/-1">
            ...
            <select v-model="form.reference_doctype" class="rec-select">
          ...
          <div class="rec-field" style="grid-column:1/-1">
            ...
            <SearchableSelect v-model="form.reference_document" :options="refDocs" placeholder="Select document…" @search="fetchRefDocs" />
          </div>
          <div class="rec-field">
      ...
      <div class="rec-dfooter">
        ...
        <button class="rec-btn-primary" :disabled="drawerSaving" @click="saveRec">
          <span v-html="icon('check',13)"></span> {{ drawerSaving?'Saving…':'Create Subscription' }}
        </button>
      </div>
    </div>
    ...
    <div class="rec-drawer rec-view-drawer" :class="{open:viewOpen}">
      <template v-if="viewDoc">
        <div class="rec-view-head">
          <div><div class="rec-view-num">{{ viewDoc.name }}</div><div class="rec-view-sub">{{ viewDoc.reference_doctype }} · {{ freqLabel(viewDoc.frequency) }}</div></div>
...
const counts=computed(()=>({active:list.value.filter(r=>(r.status||"Active")==="Active").length,paused:list.value.filter(r=>r.status==="Paused").length,cancelled:list.value.filter(r=>r.status==="Cancelled").length}));
const filtered=computed(()=>{let r=list.value;if(activeTab.value!=="all")r=r.filter(x=>(x.status||"Active")===activeTab.value);if(search.value.trim()){const q=search.value.toLowerCase();r=r.filter(x=>(x.name||"").toLowerCase().includes(q)||(x.reference_document||"").toLowerCase().includes(q));}return r;});
const sorted=computed(()=>{const col=sortCol.value;return[...filtered.value].sort((a,b)=>{const av=a[col]??"",bv=b[col]??"";const c=String(av).localeCompare(String(bv));return sortDir.value==="asc"?c:-c;});});
function sort(col){if(sortCol.value===col)sortDir.value=sortDir.value==="asc"?"desc":"asc";else{sortCol.value=col;sortDir.value="asc";}}
function sortArrow(col){if(sortCol.value!==col)return'<span style="color:#d1d5db">⇅</span>';return sortDir.value==="asc"?"↑":"↓";}
function openNew(){Object.assign(form,{reference_doctype:"Sales Invoice",reference_document:"",frequency:"Monthly",start_date:new Date().toISOString().slice(0,10),end_date:"",submit_on_creation:1,notify_by_email:""});refDocs.value=[];drawerOpen.value=true;}
function openView(r){viewDoc.value=r;viewOpen.value=true;}
async function fetchRefDocs(q=""){try{const r=await apiLinkValues(form.reference_doctype||"Sales Invoice",q);refDocs.value=r.map(x=>({label:x.name,value:x.name}));}catch{refDocs.value=[];}}
async function saveRec(){
  if(!form.reference_document)return toast.error("Reference document is required");
  drawerSaving.value=true;
  try{const doc={doctype:"Auto Repeat",reference_doctype:form.reference_doctype,reference_document:form.reference_document,frequency:form.frequency,start_date:form.start_date,end_date:form.end_date||null,submit_on_creation:form.submit_on_creation,notify_by_email:form.notify_by_email||""};const saved=await apiSave(doc);toast.success(`Subscription ${saved?.name||""} created`);drawerOpen.value=false;await load();}
  catch(e){toast.error(e.message||"Failed to create subscription");}finally{drawerSaving.value=false;}
}
onMounted(load);
</script>
...
.rec-search-wrap{display:flex;align-items:center;gap:8px;background:#f3f4f6;border-radius:8px;padding:6px 12px;min-width:220px;}
