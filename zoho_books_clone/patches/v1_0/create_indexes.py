...
def execute():
    ...
    # GL query patterns: filter by is_cancelled, account, and composite company+date+cancelled
    _idx("tabGeneral Ledger Entry", "gl_is_cancelled",      ["is_cancelled"])
    _idx("tabGeneral Ledger Entry", "gl_account",           ["account"])
    _idx("tabGeneral Ledger Entry", "gl_co_date_cancel",    ["company", "posting_date", "is_cancelled"])
    # Stock Ledger Entry: composite for item+warehouse timeline queries
    _idx("tabStock Ledger Entry",   "sle_item_wh_date",     ["item_code", "warehouse", "posting_date"])
    _idx("tabStock Ledger Entry",   "sle_is_cancelled",     ["is_cancelled"])
    # Bin: composite lookup used by every stock movement
    _idx("tabBin",                  "bin_item_wh",          ["item_code", "warehouse"])
    frappe.db.commit()
