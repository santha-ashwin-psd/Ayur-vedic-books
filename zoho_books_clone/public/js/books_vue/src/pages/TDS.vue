...
async function load(){loading.value=true;try{const co=await resolveCompany();const f=[["company","=",co],["docstatus","=",1]];if(periodFilter.value){const[yr,mo]=periodFilter.value.split("-");f.push(["posting_date",">=",`${yr}-${mo}-01`]);f.push(["posting_date","<=",`${yr}-${mo}-${new Date(+yr,+mo,0).getDate()}`]);}const raw=await apiList("Purchase Invoice",{fields:["name","posting_date","supplier","supplier_name","net_total","total_tax","grand_total"],filters:f,limit:500,order:"posting_date desc"});list.value=raw.map(e=>({...e,party:e.supplier_name||e.supplier,base_tax_amount:flt(e.net_total),tax_amount:flt(e.total_tax)}));}catch(e){toast.error(e.message||"Failed to load TDS data");}finally{loading.value=false;}}
