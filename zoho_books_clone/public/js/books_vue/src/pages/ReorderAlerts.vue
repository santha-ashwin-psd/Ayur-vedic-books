...
async function load(){loading.value=true;try{const co=await resolveCompany();const whs=await apiList("Warehouse",{fields:["name"],filters:[["company","=",co],["is_group","=",0]],limit:200});const whNames=whs.map(w=>w.name);if(!whNames.length){list.value=[];return;}const bins=await apiList("Bin",{fields:["item_code","warehouse","actual_qty","stock_uom","reorder_level","reorder_qty"],filters:[["warehouse","in",whNames],["reorder_level",">",0]],limit:1000});list.value=bins.filter(b=>flt(b.actual_qty)<=flt(b.reorder_level)).map(b=>({...b,re_order_level:b.reorder_level,re_order_qty:b.reorder_qty}));}catch(e){toast.error(e.message||"Failed to load reorder alerts");}finally{loading.value=false;}}
