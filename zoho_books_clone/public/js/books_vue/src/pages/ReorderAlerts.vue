...
async function load(){loading.value=true;try{const co=await resolveCompany();const bins=await apiList("Bin",{fields:["item_code","item_name","item_group","warehouse","actual_qty","re_order_level","re_order_qty","stock_uom"],filters:[["company","=",co],["re_order_level",">",0]],limit:500,order:"actual_qty asc"});list.value=bins.filter(b=>flt(b.actual_qty)<=flt(b.re_order_level));}catch(e){toast.error(e.message||"Failed to load reorder alerts");}finally{loading.value=false;}}
